...
function twentyseventeen_time_link() {
	...
	return sprintf(
		...
		__( '<span class="screen-reader-text">Posted on</span> %s', 'sprout' ),
				...
				if ( ( $categories_list && twentyseventeen_categorized_blog() ) || $tags_list ) {
					echo '<span class="cat-tags-links">';
						...
						if ( $categories_list && twentyseventeen_categorized_blog() ) {
							echo '<span class="cat-links">' . twentyseventeen_get_svg( array( 'icon' => 'folder-open' ) ) . '<span class="screen-reader-text">' . __( 'Categories', 'sprout' ) . '</span>' . $categories_list . '</span>';
						}

						if ( $tags_list ) {
							echo '<span class="tags-links">' . twentyseventeen_get_svg( array( 'icon' => 'hashtag' ) ) . '<span class="screen-reader-text">' . __( 'Tags', 'sprout' ) . '</span>' . $tags_list . '</span>';
						}
...
function twentyseventeen_edit_link() {
	...
	$link = edit_post_link(
		sprintf(
			...
			__( 'Edit<span class="screen-reader-text"> "%s"</span>', 'sprout' ),
			get_the_title()
		),
...
function twentyseventeen_front_page_section( $partial = null, $id = 0 ) {
	...
	if ( get_theme_mod( 'panel_' . $id ) ) {
		...
		echo '<article class="panel-placeholder panel sprout-panel sprout-panel' . $id . '" id="panel' . $id . '"><span class="sprout-panel-title">' . sprintf( __( 'Front Page Section %1$s Placeholder', 'sprout' ), $id ) . '</span></article>';
	}
}
