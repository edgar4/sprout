...
function twentyseventeen_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control( 'colorscheme', array(
		'type'    => 'radio',
		'label'    => __( 'Color Scheme', 'sprout' ),
		'choices'  => array(
			'light'  => __( 'Light', 'sprout' ),
			'dark'   => __( 'Dark', 'sprout' ),
			'custom' => __( 'Custom', 'sprout' ),
		),
		'section'  => 'colors',
		'priority' => 5,
	) );
	...
	$wp_customize->add_control( 'page_layout', array(
		'label'       => __( 'Page Layout', 'sprout' ),
		'section'     => 'theme_options',
		'type'        => 'radio',
		'description' => __( 'When the two column layout is assigned, the page title is in one column and content is in the other.', 'sprout' ),
		'choices'     => array(
			'one-column' => __( 'One Column', 'sprout' ),
			'two-column' => __( 'Two Column', 'sprout' ),
		),
	...
	for ( $i = 1; $i < ( 1 + $num_sections ); $i++ ) {
		...
		$wp_customize->add_control( 'panel_' . $i, array(
			...
			'label'          => sprintf( __( 'Front Page Section %d Content', 'sprout' ), $i ),
			'description'    => ( 1 !== $i ? '' : __( 'Select pages to feature in each area from the dropdowns. Add an image to a section by setting a featured image in the page editor. Empty sections will not be displayed.', 'sprout' ) ),
			'section'        => 'theme_options',
			'type'           => 'dropdown-pages',
			'allow_addition' => true,
...
function twentyseventeen_customize_preview_js() {
	wp_enqueue_script( 'sprout-customize-preview', get_theme_file_uri( '/assets/js/customize-preview.js' ), array( 'customize-preview' ), '1.0', true );
}
...
function twentyseventeen_panels_js() {
	wp_enqueue_script( 'sprout-customize-controls', get_theme_file_uri( '/assets/js/customize-controls.js' ), array(), '1.0', true );
}
